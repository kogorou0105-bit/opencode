...
```bash
./packages/opencode/dist/yink-<platform>/bin/yink
```
...
### Understanding bun dev vs yink

During development, `bun dev` is the local equivalent of the built `yink` command. Both run the same CLI interface:
...
# Production
yink --help          # Show all available commands
yink serve           # Start headless API server
yink web             # Start server + open web interface
yink <directory>     # Start TUI in specific directory
```
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/opencode ./src/index.ts serve --port 4096`,
    then attach TUI with `yink attach http://localhost:4096`
